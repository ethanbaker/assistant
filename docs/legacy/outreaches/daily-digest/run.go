...
import (
	...
	"github.com/ethanbaker/assistant/internal/api/modules/base/session"
...
func CreateDailyDigest(_ context.Context) *outreach.TaskReturn {
	var output string
	var err error

	// Run generator for a given number of times
	for range RETRY_LIMIT {
		output, err = getDailyDigest()
...
func getDailyDigest() (string, error) {
	var output string
...
func getNewsSection() (string, error) {
	// Make an in-memory session for a one time call
	store := session.NewInMemoryRepository()
	...
	// Make a search agent
	agent, err := search.NewSearchAgent()
	if err != nil {
