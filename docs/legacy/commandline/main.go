...
import (
	...
	"github.com/ethanbaker/assistant/internal/repository/memory"
	"github.com/ethanbaker/assistant/internal/repository/session"
...
func main() {
	...
	memoryStore, err := memory.NewMySQLRepository(dbConfig.FormatDSN())
	if err != nil {
		log.Fatalf("[COMMANDLINE]: Failed to initialize memory store: %v", err)
	}

	sessionStore, err := session.NewMySQLRepository(dbConfig.FormatDSN())
	if err != nil {
