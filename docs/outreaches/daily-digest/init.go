...
func Init() error {
	token := config.GetenvValue("NOTION_API_TOKEN")
	if token == "" {
	...
	NORMAL_TASKS.ID = config.GetenvValue("NOTION_DATABASE_TASKS_ID")
	if NORMAL_TASKS.ID == "" {
		return fmt.Errorf("NOTION_DATABASE_TASKS_ID environment variable is not set")
	}

	CRITICAL_TASKS.ID = config.GetenvValue("NOTION_DATABASE_TASKS_ID")
	if CRITICAL_TASKS.ID == "" {
		return fmt.Errorf("NOTION_DATABASE_TASKS_ID environment variable is not set")
	}

	SCHEDULE_ITEMS.ID = config.GetenvValue("NOTION_DATABASE_SCHEDULE_ID")
	if SCHEDULE_ITEMS.ID == "" {
		return fmt.Errorf("NOTION_DATABASE_SCHEDULE_ID environment variable is not set")
	}

	RECURRING_TASKS.ID = config.GetenvValue("NOTION_DATABASE_RECURRING_ID")
	...
	newsPromptPath := config.GetenvValue("OUTREACH_DAILY_DIGEST_NEWS_PROMPT")
	if newsPromptPath == "" {
	...
	calendarPath := config.GetenvValue("OUTREACH_CALENDAR_CONFIG_FILE")
	if calendarPath == "" {
	...
	searxngUrl := config.GetenvValue("SEARXNG_URL")
	if searxngUrl == "" {
